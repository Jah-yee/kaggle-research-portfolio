#!/usr/bin/env python3
"""Deterministic three-view Qwen3-VL validation on recovered HARn clips."""
...
MODALITIES = ("Depth_Color", "IR", "Depth")
PROMPT_TEMPLATE = """You are given three synchronized non-RGB videos of the same human action, in this order: depth-colorized, infrared, and raw depth. Use evidence shared across the views.
...
def main() -> None:
    ...
    parser.add_argument("--run-name", default="qwen3_vl_4b_multiview_balanced18_v1")
    ...
    eligible = qa[
        (qa["source"] == "HARn")
        & (qa["category"] == "single")
        & qa["path"].isin(complete)
    ].copy()
    selected = select_rows(eligible, args.limit, args.qa_id)
    if selected.empty:
        raise ValueError("No eligible three-view rows")
    ...
    single_view = pd.DataFrame(
        ...
        if line.strip()
    )[["qa_id", "prediction", "correct"]].rename(
        columns={"prediction": "single_view_prediction", "correct": "single_view_correct"}
    )
    matched = results.merge(single_view, on="qa_id", validate="one_to_one")
    report = {
        "experiment": args.run_name,
        "task": "zero-shot synchronized Depth_Color+IR+Depth HARn single recognition",
        "selection": "same frozen action-first, then subject-spread selector as the single-view baseline; all three modalities required",
        "seed": SEED,
        "fps_per_view": FPS,
        "max_tokens": MAX_TOKENS,
        "temperature": 0.0,
        ...
        "agreement_with_single_view": int((matched["prediction"] == matched["single_view_prediction"]).sum()),
        "multiview_only_correct": int((matched["correct"] & ~matched["single_view_correct"].astype(bool)).sum()),
        "single_view_only_correct": int((~matched["correct"] & matched["single_view_correct"].astype(bool)).sum()),
