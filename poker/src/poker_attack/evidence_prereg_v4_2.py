"""Hash-locked, canonical-root executor for EVP-DRAFT-04.2.

This version preserves the frozen statistical method while repairing only the
result-root, consumption, and Stage-2 terminal-control barriers. No real
execution is currently authorized; the guard requires future canonical v4.2
containment and independent source-audit receipts that do not yet exist.
"""
...
from .evidence_prereg_v4_2_core import (
    B001_FULL_FEATURES,
    B001_TIME_FEATURES,
    C_GRID,
    FAMILIES,
...
STATIC_CANONICAL_SHA256 = {
    ...
    "v4_1_delta": V4_1_DELTA_SHA256,
    "delta": V4_2_DELTA_SHA256,
    ...
    "v4_1_containment": "144800fbb7bda2c7f3dc5bd52e6f4fa3a43834815bb302ed3561e319108d3adb",
    "result_root_binding": V4_2_RESULT_ROOT_BINDING_SHA256,
...
STAGE_ARTIFACTS = {
    1: (
        "EVP_v4_2_stage_1_full_routes",
        "EVP_v4_2_stage_1_selected_c.json",
        "EVP_v4_2_stage_1_transforms.json",
        "EVP_v4_2_stage_1_pairwise_queries.parquet",
        "EVP_v4_2_stage_1_full_scores.parquet",
        "EVP_v4_2_stage_1_full_top_five.parquet",
        "EVP_v4_2_stage_1_reporting_family.parquet",
        "EVP_v4_2_stage_1_primary.json",
    ),
    2: (
        "EVP_v4_2_stage_2_pairwise_queries.parquet",
        "EVP_v4_2_stage_2_l0_scores.parquet",
        "EVP_v4_2_stage_2_l0_top_five.parquet",
        "EVP_v4_2_stage_2_attribution.json",
    ),
    3: (
        "EVP_v4_2_stage_3_time_routes",
        "EVP_v4_2_stage_3_selected_c.json",
        "EVP_v4_2_stage_3_transforms.json",
        "EVP_v4_2_stage_3_pairwise_queries.parquet",
        "EVP_v4_2_stage_3_eligibility.parquet",
        "EVP_v4_2_stage_3_time_scores.parquet",
        "EVP_v4_2_stage_3_time_top_five.parquet",
        "EVP_v4_2_stage_3_reporting_family.parquet",
        "EVP_v4_2_stage_3_bidirectional.json",
    ),
}
...
def _fsync_directory_v4_2(path: Path) -> None:
...
def _publish_exclusive_v4_2(path: Path, writer: Callable[[Path], None]) -> None:
    ...
    try:
        ...
        _fsync_directory_v4_2(path.parent)
    finally:
...
def _atomic_json(path: Path, value: Mapping[str, object]) -> None:
    ...
    _publish_exclusive_v4_2(path, write)


def _atomic_parquet(path: Path, frame: pd.DataFrame) -> None:
    _publish_exclusive_v4_2(
        path, lambda temporary: frame.to_parquet(temporary, index=False)
    )


def _exclusive_result_directory_v4_2(path: Path) -> None:
    try:
...
def _runtime_versions_v4_2() -> dict[str, str]:
    packages = {}
    for name in (
        "numpy",
        "pandas",
...
def _pre_data_runtime_and_dependency_guard_v4_2() -> dict[str, object]:
    """Fail closed before authorization consumption or competition-data access."""

    observed = _runtime_versions_v4_2()
...
def _canonical_result_root_v4_2(requested_work_dir: str | Path | None) -> Path:
    """Return only the authorization-bound exact result root.

    The root and its marker are executor-constructed canonical paths. A caller
    may omit ``work_dir`` or repeat that exact absolute path, but cannot select
    any relative, normalized-equivalent, symlinked, or external directory.
    """

    project_root = _project_root().resolve(strict=True)
    expected = project_root / CANONICAL_RESULT_ROOT_RELATIVE
    if not expected.is_dir() or expected.resolve(strict=True) != expected:
        raise PermissionError("canonical v4.2 result root is missing, symlinked, or escaped")
    if requested_work_dir is not None:
        requested = Path(requested_work_dir)
        if (
            not requested.is_absolute()
            or requested != expected
            or requested.resolve(strict=True) != expected
        ):
            raise PermissionError("caller-selected result root is not the exact canonical v4.2 root")

    marker = _canonical_path("result_root_binding")
    if marker.parent != expected:
        raise PermissionError("canonical result-root marker escaped the exact result root")
    document = _read_canonical_json(
        "result_root_binding", V4_2_RESULT_ROOT_BINDING_SHA256
    )
    exact_document = {
        "artifact": "EVP_v4_2_canonical_result_root_binding",
        "status": "CANONICAL_RESULT_ROOT_ONLY",
        "canonical_relative_path": CANONICAL_RESULT_ROOT_RELATIVE,
        "caller_override_allowed": False,
        "symlink_allowed": False,
        "external_root_allowed": False,
        "authorization_replay_across_roots_allowed": False,
        "result_bearing_execution_authorized": False,
    }
    if document != exact_document:
        raise PermissionError("canonical result-root binding document changed")
    return expected


def _write_complete_claim_v4_2(descriptor: int, payload: bytes) -> None:
    """Write and flush a claim fully; its existence remains authoritative."""

    remaining = memoryview(payload)
    while remaining:
        written = os.write(descriptor, remaining)
        if written <= 0:
            raise OSError("consumption claim write made no progress")
        remaining = remaining[written:]
    os.fsync(descriptor)


def _consume_authorization_v4_2(
    authorization_path: Path,
    work_dir: Path,
    exact_binding: Mapping[str, object],
    runtime_receipt: Mapping[str, object],
) -> dict[str, object]:
    canonical = _canonical_result_root_v4_2(work_dir)
    if work_dir != canonical:
        raise PermissionError("consumption is restricted to the exact canonical v4.2 root")
    work_dir = canonical
    claim_path = work_dir / CONSUMPTION_CLAIM_FILENAME
    receipt_path = work_dir / CONSUMPTION_FILENAME
    if claim_path.exists() or receipt_path.exists():
        raise PermissionError("the one-run v4.2 authorization was already consumed")
    _assert_paths_absent(_all_result_paths(work_dir))

    authorization_sha256 = _sha256(authorization_path)
    claim = {
        "status": "V4_2_RESULT_EXECUTION_AUTHORIZATION_CLAIMED",
        "authorization_sha256": authorization_sha256,
        "canonical_result_root": CANONICAL_RESULT_ROOT_RELATIVE,
        "rerun_allowed": False,
    }
    claim_payload = (
    ...
    try:
        ...
        raise PermissionError("the one-run v4.2 authorization was already consumed") from error
    try:
        _write_complete_claim_v4_2(descriptor, claim_payload)
    finally:
        os.close(descriptor)
        _fsync_directory_v4_2(work_dir)

    receipt = {
        "status": "V4_2_RESULT_EXECUTION_AUTHORIZATION_CONSUMED",
        "authorization_path": str(authorization_path),
        "authorization_sha256": authorization_sha256,
        "canonical_result_root": CANONICAL_RESULT_ROOT_RELATIVE,
        "canonical_result_root_binding_sha256": V4_2_RESULT_ROOT_BINDING_SHA256,
        "claim_path": str(claim_path),
        "claim_sha256": _sha256(claim_path),
        "binding": dict(exact_binding),
...
def _consumption_receipt_binding_v4_2(work_dir: Path) -> dict[str, str]:
    claim_path = work_dir / CONSUMPTION_CLAIM_FILENAME
    receipt_path = work_dir / CONSUMPTION_FILENAME
    if not claim_path.is_file():
        raise PermissionError("the authoritative v4.2 consumption claim is missing")
    receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
    if receipt.get("status") != "V4_2_RESULT_EXECUTION_AUTHORIZATION_CONSUMED":
        raise PermissionError("the v4.2 consumption receipt status changed")
    if receipt.get("claim_sha256") != _sha256(claim_path):
        raise PermissionError("the v4.2 consumption claim binding changed")
    return {
        "consumption_claim": str(claim_path),
        "consumption_claim_sha256": _sha256(claim_path),
        "consumption_receipt": str(receipt_path),
        "consumption_receipt_sha256": _sha256(receipt_path),
        "authorization_sha256": str(receipt["authorization_sha256"]),
    }


def _safe_consumption_state_binding_v4_2(work_dir: Path) -> dict[str, object]:
    """Describe even zero-byte/partial consumption artifacts without parsing."""

    binding: dict[str, object] = {}
    for label, filename in (
        ("consumption_claim", CONSUMPTION_CLAIM_FILENAME),
        ("consumption_receipt", CONSUMPTION_FILENAME),
    ):
        path = work_dir / filename
        present = path.is_file()
        binding[f"{label}_present"] = present
        if present:
            binding[label] = str(path)
            binding[f"{label}_sha256"] = _sha256(path)
    return binding


def _validate_result_execution_authorization_v4_2(
    authorization_path: Path,
    work_dir: Path,
    runtime_receipt: Mapping[str, object],
) -> dict[str, object]:
    ...
    v4_2_containment_path = _canonical_path("v4_2_containment")
    v4_2_source_audit_path = _canonical_path("v4_2_source_audit")
    v4_2_containment_sha256 = _sha256(v4_2_containment_path)
    v4_2_source_audit_sha256 = _sha256(v4_2_source_audit_path)
    v4_2_containment = json.loads(
        v4_2_containment_path.read_text(encoding="utf-8")
    )
    v4_2_source_audit = json.loads(
        v4_2_source_audit_path.read_text(encoding="utf-8")
    )
    if v4_2_containment.get("status") != "PASS_V4_2_IMPLEMENTATION_PRESENT_UNEXECUTED":
        raise PermissionError("canonical v4.2 containment is not an unexecuted PASS")
    if v4_2_source_audit.get("status") != "PASS_V4_2_SOURCE_AUDIT_UNEXECUTED":
        raise PermissionError("canonical v4.2 source audit is not an unexecuted PASS")

    core_path = Path(__file__).with_name("evidence_prereg_v4_2_core.py")
    ...
    required_receipt_binding = {
        ...
        "v4_2_core_sha256": core_sha256,
        "v4_2_runner_sha256": runner_sha256,
    }
    for name, receipt in (
        ("v4.2 containment", v4_2_containment),
        ("v4.2 source audit", v4_2_source_audit),
    ):
        binding = receipt.get("binding", {})
        if any(binding.get(key) != value for key, value in required_receipt_binding.items()):
            raise PermissionError(f"{name} does not bind the current v4.2 source bundle")
    exact_binding = {
        "v4_draft_sha256": V4_DRAFT_SHA256,
        "v4_1_delta_sha256": V4_1_DELTA_SHA256,
        "v4_2_delta_sha256": V4_2_DELTA_SHA256,
        ...
        "canonical_result_root_path": CANONICAL_RESULT_ROOT_RELATIVE,
        "result_root_binding_path": CANONICAL_RELATIVE_PATHS["result_root_binding"],
        "result_root_binding_sha256": V4_2_RESULT_ROOT_BINDING_SHA256,
        "v4_2_containment_path": CANONICAL_RELATIVE_PATHS["v4_2_containment"],
        "v4_2_containment_sha256": v4_2_containment_sha256,
        "v4_2_source_audit_path": CANONICAL_RELATIVE_PATHS["v4_2_source_audit"],
        "v4_2_source_audit_sha256": v4_2_source_audit_sha256,
        "v4_2_core_sha256": core_sha256,
        "v4_2_runner_sha256": runner_sha256,
    ...
    if authorization.get("binding") != exact_binding:
        raise PermissionError(
            "authorization binding is not the exact canonical v4.2 binding"
        )
    exact_permissions = {
    ...
    for receipt in (v4_2_containment, v4_2_source_audit):
    ...
    _consume_authorization_v4_2(
        authorization_path, work_dir, exact_binding, runtime_receipt
    )
...
class ExecutionStateV4_2:
    ...
    stage_2_development_data_access: bool = False
    ...
    stage_2_aggregate_performance_viewed: bool = False
...
def _read_frozen_json_receipt_v4_2(
    path: Path,
    expected_sha256: str,
    receipt_type: str,
) -> list[dict[str, object]]:
    ...
    if document["study"] != "EVP-DRAFT-04.2" or document["receipt_type"] != receipt_type:
...
def _run_stage_1_full_primary_v4(
    ...
    _exclusive_result_directory_v4_2(route_dir)
    score_parts = []
    route_receipts = []
    selection_receipts = []
    transform_receipts = []
    ...
    selection_binding = _raw_json_receipt_v4_2(
        work_dir / STAGE_1_SELECTED_C,
        "stage_1_selected_c",
        selection_receipts,
    )
    transform_binding = _raw_json_receipt_v4_2(
        work_dir / STAGE_1_TRANSFORMS,
        "stage_1_transforms",
        transform_receipts,
    )
    pairwise_binding = _raw_parquet_receipt_v4_2(
    ...
    report = {
        ...
        "binding": {
            **_consumption_receipt_binding_v4_2(work_dir),
            "v4_draft_sha256": V4_DRAFT_SHA256,
            "v4_1_delta_sha256": V4_1_DELTA_SHA256,
            "v4_2_delta_sha256": V4_2_DELTA_SHA256,
            "v4_2_result_root_binding_sha256": V4_2_RESULT_ROOT_BINDING_SHA256,
...
def _reload_stage_1_states_v4_2(
    full: FullBundleV4,
    ...
    selected_rows = _read_frozen_json_receipt_v4_2(
        selected_path,
        str(selected_binding.get("sha256")),
        "stage_1_selected_c",
    )
    transform_rows = _read_frozen_json_receipt_v4_2(
        transform_path,
...
def _run_stage_2_attribution_v4(
    ...
    execution.stage_2_development_data_access = True
    states = _reload_stage_1_states_v4_2(full, stage_1, work_dir)
    ...
    for state in states:
        ...
        execution.stage_2_model_fit = True
        execution.real_model_fit_performed = True
    ...
    execution.stage_2_aggregate_performance_viewed = True
    execution.aggregate_performance_viewed = True
...
def _run_stage_3_time_v4(
    ...
    eligibility_binding = _raw_parquet_receipt_v4_2(
...
def _write_exception_terminal_v4_2(
    work_dir: Path,
    execution: ExecutionStateV4_2,
    error: BaseException,
) -> None:
    ...
    consumption_state = _safe_consumption_state_binding_v4_2(work_dir)
    if consumption_state.get("consumption_claim_present") or consumption_state.get(
        "consumption_receipt_present"
    ):
        execution.authorization_consumed = True
    binding.update(consumption_state)
    _atomic_json(
...
def run_authorized_validation_v4_2(
    data_dir: str | Path,
    work_dir: str | Path | None,
    authorization_path: str | Path,
) -> dict[str, object]:
    """Consume one future v4.2 authorization and execute the frozen state machine."""

    data_root = Path(data_dir)
    output_root = _canonical_result_root_v4_2(work_dir)
    execution = ExecutionStateV4_2()
    try:
        runtime_receipt = _pre_data_runtime_and_dependency_guard_v4_2()
        execution.runtime_preflight_passed = True
        _validate_result_execution_authorization_v4_2(
        ...
        consumption_exists = any(
            (output_root / filename).is_file()
            for filename in (CONSUMPTION_CLAIM_FILENAME, CONSUMPTION_FILENAME)
        )
        if execution.authorization_consumed or consumption_exists:
            try:
                terminal = output_root / EXCEPTION_STOP_FILENAME
                if not terminal.exists():
                    _write_exception_terminal_v4_2(output_root, execution, error)
            except BaseException as journal_error:
                raise RuntimeError(
                    "v4.2 execution failed and its terminal STOP journal could not be written"
                ) from journal_error
        raise
...
def main() -> None:
    ...
    result = run_authorized_validation_v4_2(
