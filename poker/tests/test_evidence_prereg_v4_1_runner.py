...
from poker_attack.evidence_prereg_v4_1 import (
    ...
    run_authorized_validation_v4_1,
)
...
class SourceSeparationV4Test(unittest.TestCase):
    def test_v4_never_imports_rejected_v3_modules(self):
        for path in (
            ...
            Path(runner.__file__).with_name("evidence_prereg_v4_1_core.py"),
        ):
    ...
    def test_preflight_precedes_authorization_and_every_data_loader(self):
        source = Path(runner.__file__).read_text()
        tree = ast.parse(source)
        run = next(
            node
            ...
            and node.name == "run_authorized_validation_v4_1"
        )
        segment = ast.get_source_segment(source, run)
        self.assertLess(
            segment.index("_pre_data_runtime_and_dependency_guard_v4_1"),
            segment.index("_validate_result_execution_authorization_v4"),
        )
        self.assertLess(
            segment.index("_validate_result_execution_authorization_v4"),
    ...
    def test_append_only_and_frozen_reload_are_source_enforced(self):
        source = Path(runner.__file__).read_text()
        self.assertNotIn(".replace(path)", source)
        self.assertIn("os.link(temporary, path)", source)
        self.assertNotIn("stage_1.states", source)
        self.assertIn("_reload_stage_1_states_v4_1", source)
        tree = ast.parse(source)
        structural = next(
            node
            for node in tree.body
            if isinstance(node, ast.FunctionDef)
            and node.name == "_validate_structural_coverage_v4"
        )
        self.assertNotIn("behavior_family", ast.get_source_segment(source, structural))
...
class AuthorizationLockV4Test(unittest.TestCase):
    ...
    def test_implementation_only_stops_before_any_canonical_lookup(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(PermissionError, "implementation-only"):
                run_authorized_validation_v4_1(root / "no_data", work, authorization)
    ...
    def _canonical_project(self, root: Path):
        ...
        core_sha = _sha(Path(runner.__file__).with_name("evidence_prereg_v4_1_core.py"))
        ...
        required_binding = {
            ...
            "v4_1_core_sha256": core_sha,
            "v4_1_runner_sha256": runner_sha,
        }
        controls = {
        ...
        containment = root / CANONICAL_RELATIVE_PATHS["v4_1_containment"]
        ...
        source_audit = root / CANONICAL_RELATIVE_PATHS["v4_1_source_audit"]
        source_audit.write_text(
            json.dumps(
                {
                    "status": "PASS_V4_1_SOURCE_AUDIT_UNEXECUTED",
                    "binding": required_binding,
                    "controls": controls,
                },
                sort_keys=True,
        ...
        exact_binding = {
            ...
            "v4_1_containment_sha256": _sha(containment),
            "v4_1_source_audit_path": CANONICAL_RELATIVE_PATHS["v4_1_source_audit"],
            "v4_1_source_audit_sha256": _sha(source_audit),
            "v4_1_core_sha256": core_sha,
            "v4_1_runner_sha256": runner_sha,
            "runtime_versions": runner._runtime_versions_v4_1(),
            "schema_sha256": _sha(root / CANONICAL_RELATIVE_PATHS["schema"]),
        }
    ...
    def test_valid_synthetic_documents_consume_before_missing_data(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertTrue((work / runner.EXCEPTION_STOP_FILENAME).exists())
...
class SequentialBarrierV4Test(unittest.TestCase):
    def _run_with_mocked_stages(self, stage_1, stage_2=None, stage_3=None):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                result = run_authorized_validation_v4_1(root / "data", work, authorization)
    ...
    def test_exception_after_authorization_writes_complete_terminal_stop(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            work = root / "work"
            work.mkdir()
            ...
            authorization.write_text("{}")
            with (
                patch.object(runner, "_pre_data_runtime_and_dependency_guard_v4_1", return_value={"runtime_versions": {}, "schema_sha256": "x"}),
                patch.object(runner, "_validate_result_execution_authorization_v4", return_value={}),
                patch.object(runner, "_load_coverage_book_canonical", return_value=MagicMock()),
                patch.object(runner, "_load_full_inputs_v4", side_effect=RuntimeError("synthetic stop")),
            ):
                with self.assertRaisesRegex(RuntimeError, "synthetic stop"):
                    run_authorized_validation_v4_1(root / "data", work, authorization)
            terminal = json.loads(
                (work / runner.EXCEPTION_STOP_FILENAME).read_text()
            )
            self.assertEqual(terminal["status"], "STOP_EXCEPTION_UNEXECUTED_LATER_STAGES")
            controls = terminal["controls"]
            for name in (
                "stage_1_development_data_access",
                "stage_1_route_fit",
                "stage_1_score_serialization",
                "stage_1_aggregate_metric_computation",
                "stage_1_family_reporting_join",
                "stage_1_gate_evaluated",
...
class AppendOnlyReceiptV4_1Test(unittest.TestCase):
...
class SyntheticModelCompositionV4Test(unittest.TestCase):
    def _bundle(self) -> FullBundleV4:
        ...
        return FullBundleV4(
            ...
            reporting_families=pd.DataFrame(
                {
                    "pair_id": [f"q{index}" for index in range(45)],
                    "behavior_family": [FAMILIES[index % 3] for index in range(45)],
                }
            ),
    ...
    def test_full_nested_selection_l1_h0_and_l0_compose_without_heldout_family(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            selected_c, means, selection, transforms, inner_pairwise = _select_c_for_outer_v4(
            ...
            self.assertNotIn("behavior_family", inner_pairwise)
            score, state, diagnostics, l1_pairwise = _fit_outer_l1_h0_v4(
            ...
            self.assertNotIn("behavior_family", l1_pairwise)
            l0, l0_diagnostics, l0_pairwise = _fit_outer_l0_from_state_v4(state)
            self.assertNotIn("behavior_family", l0)
            self.assertTrue(l0["l0_score"].notna().all())
            self.assertTrue(l0_diagnostics["matched_c"])
            self.assertNotIn("behavior_family", l0_pairwise)
