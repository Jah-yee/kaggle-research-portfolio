...
"""Parent-independent, bounded last-nine-turn physical closure planner.
...
def simulate(obs, config, schedule, *, final_liquidate=False, detailed=False):
    ...
    if step < START or step + len(schedule) - 1 > FINAL or not schedule:
        raise Unsupported("outside 710..718; no day boundary or terminal auto-drop")
    ...
    for offset, action in enumerate(executed):
        ...
        if final_liquidate and t == FINAL:
            action["market"] = liquidation(pre_market, action.get("market", []), order_cap)
...
def plan_terminal(obs, config, baseline_remaining, *, max_simulations=64, passes=1, proposals_per_actor=4):
    """At 710 accept a nine-action baseline; return a guarded plan or explicit abstention."""
    begun = perf_counter()
    fallback = {"accepted": False, "reason": "", "actions": None, "simulations": 0}
    try:
        if int(_get(obs, "step", -1)) != START or len(baseline_remaining) != 9:
            raise Unsupported("planning requires step 710 and exactly nine actions through 718")
        ...
        # Final SELL differs, so certify physical dominance against unchanged-market run.
        ...
        accepted = final["actions"] != list(baseline_remaining)
        ...
        plan = {"accepted": accepted, "reason": "joint physical dominance" if accepted else "no improvement",
                ...
                "certificate": {"baseline_rows": baseline["rows"], "physical_rows": physical["rows"],
                                ...
                                "markets_710_717_unchanged": all(final["actions"][i].get("market", []) == baseline_remaining[i].get("market", []) for i in range(8))}}
...
def _recover_observed(obs, config, parent_action, plan):
    ...
    if int(_get(obs, "step")) == FINAL:
        ...
        action["market"] = []
        action = simulate(obs, config, [action], final_liquidate=True)["actions"][0]
