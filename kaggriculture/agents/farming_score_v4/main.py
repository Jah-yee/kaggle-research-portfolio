...
def _quantity(action):
    try:
        return max(1, int(action[2])) if len(action) >= 3 else 1
...
class Agent:
    ...
    def _block_requirements(self, obs, farm, step):
        """Conservative purchase budget and starting item reserve for one block."""
        ...
        hires_by_day = {}
        purchase_budget = 0.0
        end = min(len(route), step + BLOCK_TURNS)

        for turn in range(step, end):
            planned = route[turn]
            units = [planned.get("farmer") or ["PASS"], *(planned.get("hands") or [])]
            for operation in units:
                if not operation:
                    continue
                name = operation[0]
                item = operation[1] if len(operation) >= 2 else None
                if name == "FEED":
                    item = "WHEAT"
                elif name == "FERTILIZE":
                    item = "FERTILIZER"
                elif not (name == "PLACE" and item in ANIMALS):
                    continue
                item_balance[item] = item_balance.get(item, 0) - 1
                item_need[item] = max(item_need.get(item, 0), -item_balance[item])
            ...
            for order in planned.get("market") or []:
                ...
                if name == "HIRE":
                    ...
                    if 0 <= extra < len(LAND_COST):
                        purchase_budget += LAND_COST[extra]
                        quadrants += 1
                elif name == "BUY_SEED" and item in SEED_PRICE:
                    purchase_budget += SEED_PRICE[item] * quantity
                elif name == "BUY_PRODUCT" and item in ("WHEAT", "FERTILIZER"):
                    purchase_budget += max(0, int(prices.get(item, 0) or 0)) * quantity
                    item_balance[item] = item_balance.get(item, 0) + quantity
                elif name == "BUY_ANIMAL" and item in ANIMAL_COST:
                    purchase_budget += ANIMAL_COST[item] * quantity
                    item_balance[item] = item_balance.get(item, 0) + quantity

        first_hire = max(0, int(farm.get("hires_today", 0) or 0))
        for day, count in hires_by_day.items():
            offset = first_hire if day == 0 else 0
            for index in range(count):
                purchase_budget += _fib(offset + index)
        return purchase_budget, item_need, end

    @staticmethod
    def _merge_sale(market, item, quantity):
        for order in market:
    ...
    def _budget_guard(self, obs, farm, invs, shed, prices, market, step):
        """Fund the next block only when planned cash coverage is insufficient."""
        if step % BLOCK_TURNS != 0:
            return market
        purchase_budget, item_need, end = self._block_requirements(obs, farm, step)
        cash = float(farm.get("money", 0) or 0)
        for item in PRODUCTS:
            planned = max(0, self.future_sells(item, step) - self.future_sells(item, end))
            cash += min(max(0, int(shed.get(item, 0) or 0)), planned) * max(
                0, int(prices.get(item, 0) or 0)
            )
        shortfall = purchase_budget - cash
        if shortfall <= 0:
            return market
        ...
        for order in result:
            if order and order[0] == "SELL" and len(order) >= 3:
                existing[order[1]] = existing.get(order[1], 0) + max(0, int(order[2] or 0))
        in_hands = {
            item: sum(max(0, int((inventory or {}).get(item, 0) or 0)) for inventory in invs)
            for item in PRODUCTS
        }
        candidates = []
        for index, item in enumerate(PRODUCTS):
            ...
            if price < 2:
                continue
            protected = max(0, item_need.get(item, 0) - in_hands[item])
            available = max(
                0,
                int(shed.get(item, 0) or 0) - protected - existing.get(item, 0),
            )
            if available > 0:
                candidates.append((-price, index, item, available, price))
        candidates.sort()

        changed = False
        for _negative_price, _index, item, available, price in candidates:
            if shortfall <= 0:
                break
            needed = int((shortfall + price - 1) // price)
            quantity = min(available, needed)
            if quantity > 0 and self._merge_sale(result, item, quantity):
                shortfall -= quantity * price
                changed = True
        if changed:
            sales = [order for order in result if order and order[0] == "SELL"]
            others = [order for order in result if not (order and order[0] == "SELL")]
            return sales + others
        return market
    ...
    def act(self, obs):
        ...
        acts.extend([["PASS"] for _ in range(max(0, len(positions) - len(acts)))])
        ...
        # ---- projected shed: apply shed-adjacent unit actions in unit order ----
        proj = {item: max(0, int(quantity or 0)) for item, quantity in shed.items()}
        room = max(0, SHED_CAP - sum(proj.values()))
        for i in range(min(len(acts), len(positions))):
            x, y = positions[i]
            ...
            if operation == "PICKUP" and len(action) > 1:
                ...
                taken = min(max(0, int(proj.get(item, 0) or 0)), quantity)
                if taken > 0:
                    proj[item] -= taken
                    room += taken
            elif operation == "DROP" and inv:
                for item, quantity in inv.items():
                    taken = min(max(0, int(quantity or 0)), room)
                    if taken > 0:
                        proj[item] = proj.get(item, 0) + taken
                        room -= taken
            elif operation == "PLACE" and len(action) > 1 and action[1] not in ANIMALS:
                item = action[1]
                taken = min(_quantity(action), max(0, int(inv.get(item, 0) or 0)), room)
                if taken > 0:
                    proj[item] = proj.get(item, 0) + taken
                    room -= taken

        market = self._budget_guard(obs, farm, invs, shed, prices, market, step)
        ...
        if step % 24 == 23:
            ...
            actual_existing_sells = sum(min(max(0, int(shed.get(it, 0))), n) for it, n in planned_sells.items())
        ...
        # ---- sequential sell clamp: retain slots and credit earlier buys ----
        avail = dict(proj)
        kept = []
        for raw in market:
            order = list(raw)
            if order and order[0] == "SELL" and len(order) >= 3:
                item = order[1]
                have = max(0, int(avail.get(item, 0) or 0))
                quantity = min(max(0, int(order[2] or 0)), have)
                ...
                if order and order[0] == "BUY_PRODUCT" and len(order) >= 3:
                    item = order[1]
                    avail[item] = avail.get(item, 0) + max(0, int(order[2] or 0))
        market = kept
...
def agent(obs, configuration=None):
    """Independent seat state; any failure degrades to a correctly shaped PASS."""
    del configuration
    try:
        seat = int(obs.get("player", 0) or 0)
        raw_step = obs.get("step")
        step = int(raw_step) if raw_step is not None else int(obs.get("day", 0)) * 24 + int(obs.get("hour", 0))
        session = _SESSIONS.get(seat)
        if session is None or step == 0 or step < session["last_step"]:
            session = {"last_step": -1, "agent": Agent()}
            _SESSIONS[seat] = session
        action = session["agent"].act(obs)
