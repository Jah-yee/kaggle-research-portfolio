...
# The market curve is deterministic and shared by all agents. Keeping the
# curve here allows a narrow execution-layer improvement without changing the
# production route or public-state branch decisions.
_MARKET_PARAMS = {
...
def _curve(name, value):
    ...
    if name == "sqrt":
        return value ** 0.5
    if name == "log":
        return __import__("math").log1p(value)
    raise ValueError(name)


def _market_price(item, inventory):
    base, equilibrium, scale, below_name, below_target, above_name, above_target = _MARKET_PARAMS[item]
    if inventory < equilibrium:
        amplitude = below_target * base / _curve(below_name, scale)
        price = base + amplitude * _curve(below_name, equilibrium - inventory)
    else:
        amplitude = above_target * base / _curve(above_name, scale)
        price = base - amplitude * _curve(above_name, inventory - equilibrium)
    return max(1, int(round(price)))


def _rank_sell_orders(market, prices, inventory):
    """Order SELL slots by the visible curve impact; preserve all other slots."""
    rows = []
    for index, order in enumerate(market):
        if not (order and order[0] == "SELL" and len(order) >= 3 and order[1] in _MARKET_PARAMS):
            continue
        try:
            quantity = max(0, int(order[2]))
        except (TypeError, ValueError):
            continue
        current_inventory = int(inventory.get(order[1], 10000) or 0)
        current_quote = float(prices.get(order[1]) or _market_price(order[1], current_inventory))
        later_quote = float(_market_price(order[1], current_inventory + quantity))
        impact = quantity * max(0.0, current_quote - later_quote)
        rows.append((impact, index, list(order)))
    if len(rows) < 2 or max(row[0] for row in rows) <= 0:
        return market
    rows.sort(key=lambda row: (-row[0], row[1]))
    ranked = iter(row[2] for row in rows)
    return [
        next(ranked) if order and order[0] == "SELL" and len(order) >= 3 and order[1] in _MARKET_PARAMS else order
        for order in market
    ]
