...
# Micro candidate: conservative mirror-only SELL queue reorder.
_MIRROR_PARENT = agent
_MIRROR_REPORT = {'mirror_reorders':0,'mirror_modeled_gain':0,'mirror_errors':0}

def _mirror_price_params(observation):
...
def _mirror_revenue(observation, own_orders, rival_orders, stock):
    inventory = dict(observation['market']['inventory'])
    params = _mirror_price_params(observation)
    ...
    for index in range(max(len(own_orders), len(rival_orders))):
        ...
        for side,orders in enumerate((own_orders,rival_orders)):
            if index>=len(orders): pair.append(None); continue
            order=orders[index]
            if not order or len(order)<3 or order[0]!='SELL': pair.append(None); continue
            item=order[1]; q=min(max(0,int(order[2])),max(0,int(stocks[side].get(item,0))))
            pair.append([item,q])
        while any(cur is not None and cur[1]>0 for cur in pair):
            sold=[]
            for side,cur in enumerate(pair):
                if cur is None or cur[1]<=0: continue
                item=cur[0]; quote=_r37_market_price(item,inventory[item],params)
                if side==0: revenue+=quote
                cur[1]-=1;stocks[side][item]=max(0,stocks[side].get(item,0)-1);sold.append((item,quote))
            for item,quote in sold:
                if quote>1: inventory[item]+=1
    return revenue

def _mirror_reorder(observation, action):
    step=int(observation['step']); player=int(observation['player']); rival=1-player
    orders=action.get('market') or []; probe=_R44_PROBES.get(player) or {}
    if not (336<=step<696 and step%72==71 and probe.get('matched')): return action
    if observation['farms'][player]['money']>observation['farms'][rival]['money']+500: return action
    if _r37_similarity(observation)<0.90 or len(orders)<2: return action
    if any(not o or len(o)<3 or o[0]!='SELL' or o[1] not in PRODUCTS or type(o[2]) is not int or o[2]<0 for o in orders): return action
    stock=projected_shed(action,FarmView(observation))
    if not any(min(o[2],max(0,int(stock.get(o[1],0))))>0 for o in orders): return action
    rival_orders=copy.deepcopy(orders); current=copy.deepcopy(orders)
    baseline=_mirror_revenue(observation,current,rival_orders,stock); score=baseline
    for _ in range(10):
        best_score=score; best_orders=None
        for left in range(len(current)):
            for right in range(left+1,len(current)):
                if current[left]==current[right]: continue
                trial=copy.deepcopy(current);trial[left],trial[right]=trial[right],trial[left]
                trial_score=_mirror_revenue(observation,trial,rival_orders,stock)
                if trial_score>best_score: best_score,best_orders=trial_score,trial
        if best_orders is None: break
        current,score=best_orders,best_score
    gain=score-baseline
    if gain<25 or current==orders:return action
    _MIRROR_REPORT['mirror_reorders']+=1;_MIRROR_REPORT['mirror_modeled_gain']+=gain
    result=copy.deepcopy(action);result['market']=current;return result

def agent(observation,configuration=None):
    result=_MIRROR_PARENT(observation,configuration)
    try:
        standard=configuration is None or all(configuration.get(k,v)==v for k,v in [('boardSize',10),('turnsPerDay',24),('shedCapacity',100),('maxMarketOrdersPerTurn',10),('farmHandCostMult',1)])
        if standard: result=_mirror_reorder(observation,result)
    except Exception:
        _MIRROR_REPORT['mirror_errors']+=1
    _MIRROR_REPORT.update(getattr(_MIRROR_PARENT,'telemetry',{}))
    return result
agent.telemetry=_MIRROR_REPORT
agent=globals().pop('agent')
