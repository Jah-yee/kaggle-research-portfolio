...
class CFG:
    dataset_path = PROJECT_ROOT / "data" / "raw" / "rogii-wellbore-geology-prediction"
    artifacts_path = PROJECT_ROOT / "data" / "raw" / "artifacts"
    output_path = PROJECT_ROOT / "submissions" / "nihilistic_dwt_lgb4_baseline.csv"
    scores_path = PROJECT_ROOT / "reports" / "nihilistic_dwt_lgb4_baseline_scores.csv"
    seed = 42
    n_splits = 5
...
def read_train_artifact(path):
    cols = pd.read_csv(path, nrows=0).columns
    dtype = {c: np.float32 for c in cols if c not in {"well", "id"}}
    dtype["well"] = "category"
    dtype["id"] = "string"
    return pd.read_csv(path, dtype=dtype)

if (CFG.artifacts_path / "data" / "train.csv").exists():
    train_df = read_train_artifact(CFG.artifacts_path / "data" / "train.csv")
else:
...
lgb_params = [
    ...
    dict(learning_rate=0.025, n_estimators=8000, seed=2025, **lgb_params_base),
]
...
study.optimize(objective, n_trials=500, n_jobs=NCPU)
...
sub[['id', 'tvt']].to_csv(CFG.output_path, index=False)
print(f"Wrote {CFG.output_path} with shape={sub.shape}")
fold_scores_df = pd.DataFrame(fold_scores)
overall_scores_df = pd.DataFrame({k: [v] for k, v in overall_scores.items()}).transpose().sort_values(by=0, ascending=True)
CFG.scores_path.parent.mkdir(parents=True, exist_ok=True)
overall_scores_df.to_csv(CFG.scores_path)
